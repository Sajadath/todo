    ...
    <div className="relative mt-2 grow ">
      ...
      <label
        className={`absolute  text-xs transition-all duration-300 px-3 -translate-y-1/2   select-none ${
          isBeingFocused
            ? "top-0 text-indigo-500 dark:text-indigo-400 left-2"
            : props.value
            ? "top-0 text-gray-500 dark:text-gray-400 left-2"
            : "top-1/2 text-gray-500 dark:text-gray-400 left-3"
        }  px-2 bg-[#f7f9fa] dark:bg-gray-800 cursor-text`}
        htmlFor="task-title"
      >
        {props.placeholder}
      </label>
      <input
        ...
        className={`w-full border  border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 transition-all  duration-500 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:placeholder:text-indigo-300 dark:focus:placeholder:text-indigo-500 focus:outline-none ${customClassName}`}
      />
    </div>
  );
}
